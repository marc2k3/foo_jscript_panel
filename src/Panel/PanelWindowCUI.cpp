...
namespace
{
	...
	public:
		...
		IGdiFont* get_font_ui(size_t type) override
		{
			...
			{
				HFONT hFont = m_fonts_manager->get_font(t);
				if (hFont)
				{
					auto font = std::make_unique<Gdiplus::Font>(m_hdc, hFont);
					if (ensure_gdiplus_object(font))
					{
						return new ComObjectImpl<GdiFont>(std::move(font), hFont);
					}
				}
			}
			return nullptr;
		...
		LRESULT on_message(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) override
		{
			...
			{
			case WM_CREATE:
				m_colours_manager->register_common_callback(this);
				m_fonts_manager->register_common_callback(this);
				break;
			case WM_DESTROY:
				m_colours_manager->deregister_common_callback(this);
				m_fonts_manager->deregister_common_callback(this);
				break;
			case WM_SETCURSOR:
				return 1;
			case WM_ERASEBKGND:
	...
	private:
		...
		static_api_ptr_t<cui::colours::manager> m_colours_manager;
		static_api_ptr_t<cui::fonts::manager> m_fonts_manager;
