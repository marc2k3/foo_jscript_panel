...
namespace helpers
{
	static GUID artid_to_guid(size_t art_id)
	{
		if (art_id < guids::art.size())
		{
			return *guids::art[art_id];
		}
	...
	static IGdiBitmap* album_art_data_to_bitmap(const album_art_data_ptr& data)
	{
		...
		if (stream.is_valid())
		{
			...
			else if (is_webp(ptr, bytes))
			{
				return webp_to_bitmap(ptr, bytes);
			}
		}
		return nullptr;
	}

	static IGdiBitmap* get_album_art(const metadb_handle_ptr& handle, size_t art_id, bool need_stub, bool no_load, pfc::string_base& image_path)
	{
		const GUID what = artid_to_guid(art_id);
		...
		if (data.is_valid())
		{
			...
			if (!no_load)
			{
				return album_art_data_to_bitmap(data);
			}
		}
		return nullptr;
	}

	static IGdiBitmap* get_album_art_embedded(stringp path, size_t art_id)
	{
		const GUID what = artid_to_guid(art_id);
		...
		if (album_art_extractor::g_get_interface(ptr, path))
		{
			try
			{
				...
				if (data.is_valid()) return album_art_data_to_bitmap(data);
			}
			catch (...) {}
		}
		return nullptr;
	...
	static IGdiBitmap* load_image(const std::wstring& path)
	{
		pfc::com_ptr_t<IStream> stream;
