...
public:
	...
	Embed(metadb_handle_list_cref handles, size_t art_id, const album_art_data_ptr& data) : m_handles(handles), m_art_id(art_id), m_data(data), m_action(actions::attach) {}
	Embed(metadb_handle_list_cref handles, size_t art_id) : m_handles(handles), m_art_id(art_id), m_action(actions::remove) {}
	...
	{
		const GUID what = helpers::artid_to_guid(m_art_id);
