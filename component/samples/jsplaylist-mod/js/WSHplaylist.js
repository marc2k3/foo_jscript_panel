					...
					if (this.rating_hover) {
						...
						if (this.tracktype < 2) {
							...
							if (this.l_mood != this.mood) {
								...
								handles.UpdateFileInfoFromJSON(JSON.stringify({"MOOD" : utils.TimestampToDateString(t)}));
...
oList = function (object_name, playlist) {
	...
	this.contextMenu = function (x, y, id, row_id) {
		...
		_menu.AppendMenuItem(playlist_can_add_items(this.playlist) && fb.CheckClipboardContents() ? MF_STRING : MF_GRAYED, 1005, "Paste");
		_menu.AppendMenuSeparator();
		...
		case 1005:
			...
			plman.UndoBackup(this.playlist);
			plman.InsertPlaylistItems(this.playlist, base, fb.GetClipboardContents());
			break;
		default:
