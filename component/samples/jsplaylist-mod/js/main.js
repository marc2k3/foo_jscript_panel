		...
		if (p.playlistManager.inputboxID >= 0) {
			...
			if (mask == KMask.none) {
				...
				case VK_HOME:
					...
					break;
				case VK_DELETE:
					if (playlist_can_remove_items(act_pls)) {
				...
				case KMask.ctrl:
					...
					if (vkey == 86) { // CTRL+V
						if (playlist_can_add_items(act_pls) && fb.CheckClipboardContents()) {
...
function setWallpaperImg() {
	...
	if (properties.wallpapermode > -1) {
		var img = utils.GetAlbumArtV2(metadb, properties.wallpapermode);
	} else {
		var img = gdi.Image(fb.ProfilePath + properties.wallpaperpath);
	}

	return FormatWallpaper(img, ww, wh, 2, 0, 0, "", true);
}
...
function on_drag_over(action, x, y, mask) {
	...
	} else if (playlist_can_add_items(plman.ActivePlaylist)) {
		g_dragndrop_trackId = -1;
		g_dragndrop_rowId = -1;
